/**
 * 셀러 판정 규칙 (순수 함수 + 정규식) — DB/캐시 의존 없음.
 *
 * creators.ts 에서 분리. 여기 규칙은 "이 작성자가 진성 셀러인가"를 bio URL·
 * signature·핸들·영상수만으로 판정한다. 캐시/DB 갱신은 creators.ts 가 담당.
 * 값(정규식·임계값)은 의뢰인 튜닝이 잦은 지점이라 한 파일에 모아둔다. — 값 불변.
 */
